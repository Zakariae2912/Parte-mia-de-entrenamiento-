# ============================================================
# 09_interpretacion_variables_derivadas.py
# COEFICIENTES Y ODDS RATIOS DEL MODELO
# CON VARIABLES CLÍNICAS DERIVADAS
...
# ------------------------------------------------------------
# FUNCIÓN PARA REPRESENTAR LAS ODDS RATIOS
# ------------------------------------------------------------

def crear_grafico_or_derivadas(
    resultados_or,
    hospital_train,
    ruta_salida
):

    # Ordenar las variables según su odds ratio
    ...
    figura.update_layout(
        title=(
            "Odds ratios de las variables clínicas derivadas "
            "- Hospital " + hospital_train
        ),
        xaxis_title="Odds ratio, escala logarítmica",
        yaxis_title="Variable clínica derivada",
        xaxis_type="log",
        height=650
    )
...
spark = SparkSession.builder \
    .appName("Interpretacion_Variables_Derivadas_LOSO") \
...
# --------------------------------------------------------
    ...
    coeficientes = list(
        modelo_lr.coefficients
    )

    resultados_or = []

    for nombre_variable, coeficiente in zip(
        features_validas,
        coeficientes
    ):
    ...
    resultados_or_ordenados = sorted(
        resultados_or,
        key=lambda fila: abs(fila[1]),
        reverse=True
    )

    print(
        "\nCoeficientes y odds ratios entrenando con Hospital",
        hospital_train
    )

    for variable, coeficiente, odds_ratio in resultados_or_ordenados:
...
# ------------------------------------------------------------
# TABLAS DE COEFICIENTES Y ODDS RATIOS
# ------------------------------------------------------------

tabla_or_AB = spark.createDataFrame(
    resultados_or_AB,
    [
        "variable",
        "coeficiente",
...
tabla_or_BA = spark.createDataFrame(
    resultados_or_BA,
    [
        "variable",
        "coeficiente",
...
print(
    "\nOdds ratios de las variables derivadas "
    "entrenando con Hospital A:"
)
...
# ------------------------------------------------------------
# EXPORTACIÓN DE LAS ODDS RATIOS
# ------------------------------------------------------------

ruta_output_or_derivadas = (
    "file:///home/adminp/TFA3/output/"
    "odds_ratios_variables_derivadas"
)
...
tabla_or_BA.orderBy(
    ...
    .csv(
        ruta_output_or_derivadas
        + "/02_entrena_B_valida_A"
    )

print(
    "\nTablas de odds ratios exportadas en:",
    ruta_output_or_derivadas
)

# ------------------------------------------------------------
# GRÁFICOS DE ODDS RATIOS DE LAS VARIABLES DERIVADAS
# ------------------------------------------------------------

ruta_grafico_or_derivadas_AB = (
    "/home/adminp/TFA3/output/"
    "odds_ratios_variables_derivadas/"
    "03_OR_derivadas_entrena_A_valida_B.html"
)

ruta_grafico_or_derivadas_BA = (
    "/home/adminp/TFA3/output/"
    "odds_ratios_variables_derivadas/"
    "04_OR_derivadas_entrena_B_valida_A.html"
)

crear_grafico_or_derivadas(
    resultados_or_AB,
    "A",
    ruta_grafico_or_derivadas_AB
)
...
print(
    "\nGráficos de odds ratios "
    "de las variables derivadas creados:"
)

print(ruta_grafico_or_derivadas_AB)
print(ruta_grafico_or_derivadas_BA)
# ------------------------------------------------------------
# RESUMEN
# ------------------------------------------------------------
